...
export function apply(ctx) {
  ...
  // ── reasoning effort (the harness's "推理等级" / Effort selector) ────────
  // Ollama's native /api/chat takes a top-level `think` field. Per Ollama's
  // docs, most thinking-capable open models (Qwen3, DeepSeek-R1 / v3.1) only
  // accept a boolean — thinking is an on/off switch, so we expose just the
  // `off`/`on` pair by default. Only a handful (e.g. GPT-OSS) accept levels
  // (`low`/`medium`/`high`); those opt in via a per-model `thinkingMode:
  // levels` and get the wider set. `off` maps to think:false, `on` to
  // think:true, and a level to think:"<level>".
  const THINK_BOOLEAN_EFFORTS = [
    { id: 'off', name: 'Off', description: 'Disable thinking (Ollama think: false)' },
    { id: 'on', name: 'On', description: 'Enable thinking (Ollama think: true)' }
  ]
  const THINK_LEVEL_EFFORTS = [
    { id: 'off', name: 'Off', description: 'Disable thinking (Ollama think: false)' },
    { id: 'low', name: 'Low' },
    { id: 'medium', name: 'Medium' },
    { id: 'high', name: 'High' },
    { id: 'max', name: 'Max', description: 'Highest thinking level' }
  ]
  const THINK_MODES = ['boolean', 'levels']
  const EFFORTS_BY_MODE = { boolean: THINK_BOOLEAN_EFFORTS, levels: THINK_LEVEL_EFFORTS }
  const EFFORT_IDS_BY_MODE = Object.fromEntries(
    THINK_MODES.map((mode) => [mode, EFFORTS_BY_MODE[mode].map((effort) => effort.id)])
  )
  const ALL_THINK_IDS = new Set(EFFORT_IDS_BY_MODE.boolean.concat(EFFORT_IDS_BY_MODE.levels))
  ...
  // The capability probe is best-effort and MUST never block or break the
  // model catalog: `resolveModel` is awaited by the harness per model, and a
  // thrown error (or a hang on a slow `/api/tags`) drops the whole provider
  // from the bottom-right model selector. So the probe runs in the background
  // (bounded, non-fatal) and `resolveModel` only reads an already-cached
  // snapshot. When the cache is cold — or the server's `/api/tags` is slow or
  // offline — a model is treated as non-thinking until a probe lands; the
  // manual `thinkingCapable: true` override still works.
  const CAPABILITY_TTL_MS = 5 * 60 * 1000
  const CAPABILITY_RETRY_MS = 60 * 1000
  const capabilityCache = new Map() // baseURL -> { expires, thinking:Set<modelName> }
  const capabilityInFlight = new Map() // baseURL -> Promise, dedupes concurrent /api/tags
  async function fetchCapabilities(baseURL) {
    const url = `${baseURL.replace(/\/+$/, '')}/api/tags`
    let handle
    try {
      handle = ctx.subprocess.spawn({
        argv: [await resolveCurl(ctx.subprocess), '-sS', '-f', '--max-time', '5', url, '-H', 'Accept: application/json', '-H', `User-Agent: ${USER_AGENT}`],
        cwd: '/',
    ...
    for (const m of body.models) {
      if (m !== null && typeof m === 'object' && Array.isArray(m.capabilities) && m.capabilities.includes('thinking')) {
        if (typeof m.name === 'string' && m.name.length > 0) thinking.add(m.name)
      }
    }
    return { thinking }
  }
  // Return the current capability snapshot for a base URL. Never blocks: if the
  // snapshot is cold or stale it kicks off a background probe (deduped) and
  // immediately returns the last-known snapshot (empty the first time).
  function readCapabilities(baseURL) {
    const cached = capabilityCache.get(baseURL)
    if (cached !== undefined && cached.expires > Date.now()) return cached
    if (!capabilityInFlight.has(baseURL)) {
      capabilityInFlight.set(baseURL, (async () => {
        let thinking
        try {
          thinking = (await fetchCapabilities(baseURL))?.thinking
        } catch {
          thinking = undefined
        }
        const result = { expires: Date.now() + (thinking === undefined ? CAPABILITY_RETRY_MS : CAPABILITY_TTL_MS), thinking: thinking ?? new Set() }
        capabilityCache.set(baseURL, result)
        return result
      })().finally(() => capabilityInFlight.delete(baseURL)))
    }
    return cached ?? { expires: 0, thinking: new Set() }
  }
  // Auto-detect thinking support for one model route. Synchronous, never
  // throws: reads the cached snapshot (or triggers a background probe) and
  // falls back to false, so the catalog is never blocked or failed by this.
  function modelSupportsThinking(profile, model) {
    try {
      if (profile === undefined || profile.baseURL === undefined || profile.baseURL.length === 0) return false
      return readCapabilities(profile.baseURL).thinking.has(model)
    } catch {
      return false
    }
  }
  ...
  const ENVELOPE = {
    ...
    refs: {
      ...
      '29': { type: 'string', meta: {} },
      '30': { type: 'boolean', meta: {} },
      '31': { type: 'string', meta: {} },
      '16': { type: 'object', meta: { default: {} }, dict: { id: 8, name: 9, contextWindow: 12, maxTokens: 15, reasoningEffort: 29, thinkingCapable: 30, thinkingMode: 31 } },
    ...
    for (const [route, source] of Object.entries(providersRaw)) {
      ...
      if (source.models !== undefined) {
        ...
        for (const [index, model] of source.models.entries()) {
          ...
          if (model.thinkingCapable !== undefined && typeof model.thinkingCapable !== 'boolean') {
            throw new Error(`ollama: provider "${route}" model "${id}" thinkingCapable must be a boolean`)
          }
          if (model.thinkingMode !== undefined && !THINK_MODES.includes(model.thinkingMode)) {
            throw new Error(`ollama: provider "${route}" model "${id}" thinkingMode must be one of ${THINK_MODES.join(', ')}`)
          }
          const mode = model.thinkingMode === 'levels' ? 'levels' : 'boolean'
          const effortIds = EFFORT_IDS_BY_MODE[mode]
          if (model.reasoningEffort !== undefined && !effortIds.includes(model.reasoningEffort)) {
            throw new Error(`ollama: provider "${route}" model "${id}" reasoningEffort must be one of ${effortIds.join(', ')} for thinkingMode "${mode}"`)
          }
          const next = { id }
          ...
          if (model.reasoningEffort !== undefined) next.reasoningEffort = model.reasoningEffort
          if (model.thinkingCapable !== undefined) next.thinkingCapable = model.thinkingCapable
          if (model.thinkingMode !== undefined) next.thinkingMode = model.thinkingMode
          models.push(next)
        }
      }
  ...
  function refreshProfiles() {
    ...
    profiles = next
    syncRegistrations()
    // Eagerly warm the capability cache (non-blocking) so auto-detected
    // thinking classification is ready by the time the model selector loads.
    for (const profile of next.values()) {
      if (profile?.baseURL !== undefined && profile.baseURL.length > 0) void readCapabilities(profile.baseURL)
    }
  }
  ...
  function buildAdapter(getProfilesFn, getValueFn) {
    return {
      ...
      async resolveModel(provider, model, signal) {
        ...
        // Adapter-owned reasoning efforts. A model exposes them when it is
        // thinking-capable, which is auto-detected from Ollama's per-model
        // `capabilities` (a `thinking` capability) unless a manual
        // `thinkingCapable` override says otherwise: `true` forces the control,
        // `false` hides it, and an absent value means "auto-detect". By default
        // the effort pair is the on/off switch (what most open models support);
        // models that accept granular levels set `thinkingMode: levels` and get
        // the wider set. `defaultEffort` is only materialized when the model
        // entry sets a per-model default; otherwise the selector shows "Default"
        // and the request sends no `think`.
        // modelSupportsThinking is synchronous and never throws: it reads the
        // cached capability snapshot (or kicks off a background probe) and
        // treats an unknown model as non-thinking. resolveModel therefore never
        // blocks on the server and can never reject the catalog.
        const supportsThinking = entry?.thinkingCapable === true
          ? true
          : entry?.thinkingCapable === false
            ? false
            : modelSupportsThinking(profile, model)
        if (supportsThinking) {
          const mode = entry?.thinkingMode === 'levels' ? 'levels' : 'boolean'
          const effortIds = EFFORT_IDS_BY_MODE[mode]
          info.reasoning = {
            efforts: EFFORTS_BY_MODE[mode],
            ...(entry?.reasoningEffort !== undefined && effortIds.includes(entry.reasoningEffort)
              ? { defaultEffort: entry.reasoningEffort }
              : {})
          }
        }
        return info
      ...
      async *stream(options) {
        ...
        // Reasoning effort → Ollama's top-level `think` field. `off` means
        // `think: false` (disable thinking), `on` means `think: true`, and a
        // granular level becomes `think: "<level>"` (only for level-capable
        // models). When the caller did not select an effort
        // (options.reasoningEffort is undefined) we send no `think`, so Ollama
        // keeps its own default.
        const effort = options.reasoningEffort
        if (effort !== undefined && ALL_THINK_IDS.has(effort)) {
          ...
          for (;;) {
            ...
            for (;;) {
              ...
              if (message !== null && typeof message === 'object') {
                // Ollama surfaces the reasoning trace in `message.thinking`,
                // separate from the final `message.content`. Surface it as a
                // reasoning block so the harness renders the thinking trace.
