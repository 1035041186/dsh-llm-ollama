  ...
  function buildAdapter(getProfilesFn, getValueFn) {
    return {
      ...
      resolveModel(provider, model, _signal) {
        ...
        // Adapter-owned reasoning efforts, exposed only for models marked
        // thinking-capable (`thinkingCapable: true`). By default the effort
        // pair is the on/off switch (what most open models support); models
        // that accept granular levels set `thinkingMode: levels` and get the
        // wider set. Marking a model makes the harness's bottom-right model
        // selector show a 推理等级 / Effort row for it; otherwise the row is
        // absent (a text-only model never shows it). `defaultEffort` is only
        // materialized when the model entry sets a per-model default; otherwise
        // the selector shows "Default" and the request sends no `think`.
        if (entry?.thinkingCapable === true) {
